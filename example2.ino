...
void loop(){
  ...
  if(digitalRead(buttonPin)==LOW){
    ...
    if(reverseMode && reverseCount>0){
      ...
      // 리버스 모드 전환 조건
      bool all=true; 
      for(int i=0;i<4;i++) if(horsePos[i]==0) all=false;
      if(all) reverseMode=true;
    }
    // 상태 갱신 후 다시 표시
    show7Seg();
    updateMatrix();
    showReverseCards(); // 리버스 카드 상태도 같이 표시
    // 종료 체크
    for(int i=0;i<4;i++) 
      if(horsePos[i]>=7){
        playTune();
...
void showReverseCards() {
  for (int i = 0; i < 6; i++) {
    int row = i + 1; // row 1~6
    digitalWrite(rowPins[row], HIGH); // row 선택
    if (i < reverseCount) {
      digitalWrite(colPins[7], LOW);  // 켜짐
    } else {
      digitalWrite(colPins[7], HIGH); // 꺼짐
    }
    delay(1);
    digitalWrite(rowPins[row], LOW); // 초기화
  }
}


void showCardName(Suit s) {
  switch (s) {
    case DIAMOND: Serial.println("Diamond"); break;
    case HEART:   Serial.println("Heart");   break;
    case SPADE:   Serial.println("Spade");   break;
    case CLOVER:  Serial.println("Clover");  break;
  }
}
